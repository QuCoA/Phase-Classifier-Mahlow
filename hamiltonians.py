...
class Hamiltonian:
    def __init__(self, n, spin="1"):
        ...
        self._matrix = csr_matrix(
            (utils.spin_states[spin] ** self._n, utils.spin_states[spin] ** self._n),
            dtype=np.complex128,
        )
    ...
    def _cyclical_term(self, operator: csr_matrix) -> csr_matrix:
        return csr_matrix(
            kron(operator, kron(self.kroned_identity(self.n - 2), operator))
        )
...
class BondAlternatingXXZ(Hamiltonian):
    def __init__(self, n, Delta, delta, spin="1"):
        ...
        for l in range(n - 1):
            coeff = 1 - delta * (-1) ** (l)
            self._matrix += coeff * self._build_term(
            ...
            self._matrix += (
                Delta * coeff * self._build_term(l, utils.spin_operators[spin]["Sz"])
            )  # Δ(S_l^z S_{l+1}^z) term

        # Cyclical terms
        coeff = 1 - delta * (-1) ** (n - 1)
        self._matrix += coeff * self._cyclical_term(
        ...
        self._matrix += (
            Delta * coeff * self._cyclical_term(utils.spin_operators[spin]["Sz"])
        )  # Δ (S_N^z S_1^z) term
